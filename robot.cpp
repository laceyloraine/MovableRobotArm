...
void DrawRobotArm(int NumSegs)
{
  ...
  glPushMatrix();
	...
	glRotatef(BaseSpin, 0, 1.0, 0);
	DrawBase(NumSegs);
	glTranslatef(0, 0.43f, 0);
	glRotatef(ShoulderAng, 0, 0, 1.0);
	DrawArmSegment(NumSegs);
	glTranslatef(0, 0.52f, 0);
	glRotatef(ElbowAng, 0, 0, 1.0);
	DrawArmSegment(NumSegs);
	glTranslatef(0, 0.52f, 0);
	glRotatef(WristAng, 0, WristTwistAng, 1.0);
	DrawWrist(NumSegs);
	glTranslatef(0, 0.2f, 0);

	//Draw left finger
	glPushMatrix();
		glRotatef(FingerAng1, 0, 0, 1.0);
		DrawFingerBase(NumSegs);
		glTranslatef(0, 0.3f, 0);
		glRotatef(FingerAng2, 0, 0, 1.0);
...
{
	//Select joint from numbered keys 1 - 5 and assign movement
...
void MotionFunc(int x, int y)
{
	...
	//Track mouse movements for arm control
  OldX=NewX; OldY=NewY;
  NewX=x;    NewY=y;
  ...
  if (LeftButtonDown) 
    switch(RobotControl)
    {
      case 0: BaseTransX+=RelX; BaseTransZ+=RelY; break;	//Move base
      case 1: BaseSpin+=RelX*180; break;					//Rotate base
      case 2: ShoulderAng+=RelY*-90; ElbowAng+=RelX*90; break;		//Vertical motion controls shoulder joint, Horizontal motion controls elbow joint
      case 3: WristAng+=RelY*-180; WristTwistAng+=RelX*180; break;	//Rotate wrist joint
      case 4: FingerAng1+=RelY*90; FingerAng2+=RelX*180; break;		//Vertocal motion controls knuckle joints, Horizontal motion controls finger joints
    };
}
